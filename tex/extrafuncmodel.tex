...
where
%the mapping solution $\textbf{x}$ is computed over the node's utilization (or load) $u_m(\textbf{x})$, which is computed over the tasks that are allocated on the node.

%$\textbf{x}$, , where  is the power consumption of the node $m$ with utilization $u_m(\textbf{x}). $for a solution $\textbf{x}$ as shown from Equation 
%(\textbf{x})=f_p(u_m(x))
%The In this section, we discuss the timing, application reliability, and power consumption models used throughout the paper.
