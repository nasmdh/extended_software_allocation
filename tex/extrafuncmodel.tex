...
The software allocation problem is a type of job shop scheduling with constraints, as such it is a discrete optimization problem \cite{}. The solution to the allocation problem is represented by a vectormatrix $\bx=\{\bspx{k}:k=1,...,N_a\}$, where \spx{k} is a matrix of size $N_c\times K$, and \ssx{x}{k}{ij}$=k\in \{1,…,N_m\}$ represents the mapping of the software component replica \ssx{c}{k}{ij} to the computation node $m_k$.
\begin{equation}
...
The reliability  is computed from the execution framework that is provided to run the application, which consists of computation nodes \ssp{M} and the shared CAN bus B, as shown in Equation (\ref{eqn_appreliability_app}). The nodes \ssp{M} host the components \ssp{C} and are determined by searching the mapping \xsp{k}  in polynomial time using Equation (\ref{eqn_nodes_app}).
\begin{align}
...
The application $A_k$ fails in the event $\omega \in \ttssp{\mathcal{F}}(\ttssp{M})$ if there exists a component type \sss{c} where all of its replica \sss{Q} \textit{fail}, otherwise, the application functions, as shown in Equation (\ref{eqn_app_failure}).  A component replica $\ttsss{q}[k][ij]\in \ttsss{Q}$ fails if the node that host it $m_h$ fails, where $h=\xkij$.  
\begin{align}
...
\end{align}
\end{definition}
where \sss{M}[k][\omega] denote the nodes that function at the event $\omega$, and can be found by searching the nodes \ssp{M} with state $\omega.b_e=1$ in constant time , as show in Equation (\ref{eqn_pro}).
